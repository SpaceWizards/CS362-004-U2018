...
//this randomly tests the smithy card
int main(){
  //initialize the game
  srand(time(NULL));
  ...
  for (testnum = 0; testnum < 100; testnum++){
    ...
    gs->hand[0][loc] = smithy;
    //make sure the next card isn't a smithy to avoid weird errors
    gs->hand[0][((loc + 1) % beforesize)] = mine;
    //run the card
    smithyEffect(0, loc, gs);
    //if something went wrong increment failure, otherwise success
    if(beforesize != (numHandCards(gs)+2)){
      failures++;
    }
    else if (gs->hand[0][loc] == smithy){
      failures++;
    }
    else{
      successes++;
  ...
  printf("Smithy successes/failures out of 100: %d / %d \n", successes, failures);
  return 0;
}
